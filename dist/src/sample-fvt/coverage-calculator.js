...
export class CoverageCalculator {
     ...
     */
    constructor(sampleDir, outputDir) {
     ...
     */
    async calculate(sampleName, iteration) {
        const framework = await this.detectFramework();
        if (framework === 'none') {
            throw new Error(`No test framework detected in ${this.sampleDir}. ` +
     ...
     */
    async runCoverageCommand(framework) {
        ...
        try {
            ...
            throw new Error(`Coverage command failed for framework "${framework}": ${err.message}`);
        }
    }
    /**
     * Build the shell command and arguments for running tests with coverage.
     */
    buildCoverageCommand(framework) {
        switch (framework) {
            case 'vitest':
     ...
     */
    parseCoverageOutput(framework, stdout, sampleName, iteration) {
        switch (framework) {
            case 'vitest':
            case 'c8':
     ...
     * ```
     */
    parseV8Coverage(stdout, sampleName, iteration) {
     ...
     * ```
     */
    parseNycCoverage(stdout, sampleName, iteration) {
     ...
     * ```
     */
    parsePytestCovOutput(stdout, sampleName, iteration) {
        // Find the TOTAL line
     ...
     * ```
     */
    parseCoveragePyOutput(stdout, sampleName, iteration) {
        try {
            ...
            const branchPct = branchTotal > 0 ? Math.round((branchCovered / branchTotal) * 10000) / 100 : 0;
            return {
                statements: {
                    total: stmtsTotal,
    ...
    emptyReport(sampleName, iteration) {
        return {
