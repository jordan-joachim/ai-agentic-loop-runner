...
1. Clone the IBM CodeEngine samples repository into `inputs/code-engine-samples`:

   ```bash
   git clone https://github.com/IBM/CodeEngine.git inputs/code-engine-samples
   ```

   If the repository is already present (e.g. bind-mounted by the harness),
   verify it is a valid git clone and skip the download step.

2. Install tools needed by the harness for all samples:
...
3. Capture baseline FVT coverage for every sample under
   `inputs/code-engine-samples/samples/ai/` using the detected test framework.
...
4. Write `setup/starting-summary.md` describing:
   ...
   - The number of sample projects discovered under `samples/ai/`.
...
Increase FVT line coverage for every sample under `samples/ai/` in the IBM
