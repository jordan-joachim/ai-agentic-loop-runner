...
# workspace/config/agents.json, then delegates to the harness run-fleet.sh script.
...
log() {
  echo "[run-code-engine-fleet] $*"
}

error() {
  echo "[run-code-engine-fleet] ERROR: $*" >&2
}
...
print_result_hint() {
  log ""
  log "After the fleet completes, download results from COS:"
