...
import type { SamplePlan, Plan } from '../types.js';
...
export class SampleFVTPlanner {
   ...
   */
  async discover(): Promise<SamplePlan[]> {
   ...
   */
  async emitPlans(outputDir: string): Promise<string[]> {
    ...
    for (const sample of samples) {
      ...
      const plan: Plan = this.buildPlan(sample);
   ...
   */
  private buildPlan(sample: SamplePlan): Plan {
    return {
      meta: {
