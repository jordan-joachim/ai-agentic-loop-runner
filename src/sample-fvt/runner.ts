...
export class SampleFVTRunner {
  ...
  private readonly planner: SampleFVTPlanner;
  private readonly reviewer: CoverageReviewer;
   ...
   */
  async run(outputDir: string): Promise<SampleFVTAggregateResult> {
    ...
    const aggregate: SampleFVTAggregateResult = {
  ...
  private async runSampleLoop(
    ...
    let previousCoveragePath: string | null = null;
    let finalCoveragePercent = 0;
    let finalReview: CoverageReview | undefined;
    ...
    for (let iteration = 1; iteration <= this.config.maxIterations; iteration++) {
      ...
      try {
        ...
        return {
          ...
          error: `Coverage calculation failed: ${(err as Error).message}`,
        };
      }
   ...
   */
  private async writeAggregateResult(
    outputDir: string,
    aggregate: SampleFVTAggregateResult,
  ): Promise<void> {
